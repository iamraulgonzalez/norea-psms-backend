...
require_once __DIR__ . '/../config/database.php';
    class MonthlyAttendance {
        ...
        public function fetchAll() {
            $query = "SELECT * FROM tbl_month_att WHERE isDeleted = 0";
        ...
        public function update($id, $data) {
            $query = "UPDATE tbl_month_att SET Absent_asked_per = :Absent_asked_per WHERE month_att_id = :month_att_id AND isDeleted = 0";
            $stmt = $this->conn->prepare($query);
        ...
        public function delete($id) {
            $query = "UPDATE tbl_month_att SET isDeleted = 1 WHERE month_att_id = :month_att_id";
            $stmt = $this->conn->prepare($query);
            $stmt->bindParam(':month_att_id', $id);
            return $stmt->execute();
        }
        ...
        public function fetchById($id) {
            $query = "SELECT * FROM tbl_month_att WHERE month_att_id = :month_att_id AND isDeleted = 0";
            $stmt = $this->conn->prepare($query);
            $stmt->bindParam(':month_att_id', $id);
            $stmt->execute();
